...
##inserting last activity date  
 ...
 task :insert_old_note_attachment => :environment do
  ...
  ##inserting reminder types
  task :insert_reminder_types => :environment do
    types = ["Call", "Skype", "Email", "Appointment", "Billing", "Documentation", "Follow-up", "Meeting", "None", "Quote", "Thank-you"]
     types.each do |t|
          ReminderType.create(:name => t)
          p ReminderType.count
     end
  end
