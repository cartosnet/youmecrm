...
private
  ...
  def data(_type)
    ...
    case  _type
    when "incoming","junk","qualify", "inactive_deals","all","no_contact","follow_up_required","follow_up","quoted","meeting_scheduled","forecasted","waiting_for_project_requirement","proposal","estimate","not_qualify"
     ...
     deals.includes(:last_task, :current_country,:deal_labels, :assigned_user, :initiator, :deal_status,:priority_type).includes(:deals_contacts).map do |deal|
        [
          ...
          h(deal.deals_contacts.present? ? (deal.deals_contacts.first.contactable.present? ? deal.deals_contacts.first.contactable.work_phone : "") : ""), #row [7]
          ...
          h(deal.deals_contacts.present? && deal.deals_contacts.first.contactable.present? && deal.deals_contacts.first.contactable.country.present? ? deal.deals_contacts.first.contactable.country.name : ""), #row [12]
          ...
          h(deal.deals_contacts.present? ? deal.deals_contacts.first.contactable.id : ""), #row[33]
  ...
  def fetch_deals
   ...
   # ss
    ...
    user_label_val = params[:user_label_val]  
    if(params[:q] == "1")
 ...
 def sort_column
    if(params[:_type] == "incoming" || params[:_type] == "qualify"  || params[:_type] == "no_contact" || params[:_type] == "follow_up_required" || params[:_type] == "follow_up" || params[:_type] == "quoted" || params[:_type] == "meeting_scheduled" || params[:_type] == "forecasted" || params[:_type] == "waiting_for_project_requirement" || params[:_type] == "proposal" || params[:_type] == "estimate")
	   columns = %w[deals.id title created_at country_id created_at amount amount priority_type_id]
	    ...
	    columns = %w[deals.id title created_at amount id id email priority_type_id deal_status_id country_id priority_type_id]
	   elsif params[:_type] == "un_assigned"
      columns = %w[deals.id title created_at amount id id email priority_type_id deal_status_id country_id priority_type_id]
	  elsif params[:_type] == "won"
	    columns = %w[deals.id title id created_at stage_move_date amount id priority_type_id]
