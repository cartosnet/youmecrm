...
#        deals = @current_user.organization.deals
        query_condition.where("deals.is_active=? AND deals.organization_id = ? ", true, @current_user.organization.id)
...
#     #deals=deals.where("(deals.assigned_to=? )", @current_user.id) unless @current_user.is_admin?
   ...
   #deals=@current_user.organization.deals.includes(:deal_status).where("deal_statuses.original_id IN (?) ", status)
    query_condition=[]
      if(user.is_super_admin? || user.is_admin?)
        query_condition.where("deals.is_active=? AND deals.organization_id = ? ", true, user.organization_id)
  ...
  def get_deal_index_status_count(status, user, associated_by)
    ...
    if user.present?
       if associated_by == "created by"
        query_condition.where("deals.is_active = ? and deals.initiated_by= ? and deals.organization_id = ?",true,user.id,user.organization_id)
       elsif associated_by == "assigned to"
        query_condition.where("deals.is_active = ? and deals.assigned_to = ? and deals.organization_id = ? ",true,user.id, user.organization_id)
       end
    end
    query_condition
  end
