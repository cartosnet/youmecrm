...
#*****************************************************************************************/
  ...
  skip_before_filter :authenticate_admin, :only => [:new_report, :get_selected_filter,:reload_left_side_content, :generate_report]
...
#dfgdfgdfg
  ...
  def get_selected_filter
    render partial: "get_selected_filter"
  end
  def reload_left_side_content
  ...
  def generate_report
    begin
      if params[:report_type] == "Leads"
        if params[:priority_type].present?
          priority = PriorityType.find_by_name(params[:priority_type])
          @reports = @current_organization.deals.by_priority(priority.id).order("created_at desc")
          if params[:lead_country].present?
            @reports = @reports.where("country_id =?",params[:lead_country]).order("created_at desc")
          end
        else
          @reports = @current_organization.deals.order("created_at desc")
        end
        filename = 'leads-' + Time.zone.now.strftime("%Y%m%d%I%M%S").to_s + '.csv'
      
      elsif params[:report_type] == "Tasks"
        if params[:date_type].present?
          if params[:date_type] == "Specific Date"
            date = Date.strptime(params[:report_date],"%m-%d-%Y")
            @reports = @current_organization.tasks.where(created_at: date.midnight..date.end_of_day).order("created_at desc")
          else
            first_date = Date.strptime(params[:report_date_first],"%m-%d-%Y")
            sec_date = Date.strptime(params[:report_date_sec],"%m-%d-%Y")
            @reports = @current_organization.tasks.where(created_at: first_date.midnight..sec_date.end_of_day).order("created_at desc")
          end
        else
          @reports = @current_organization.tasks.order("created_at desc")
        end
        filename = 'tasks-' + Time.zone.now.strftime("%Y%m%d%I%M%S").to_s + '.csv'
      
      elsif params[:report_type] == "Contacts"
        filename = 'contacts-' + Time.zone.now.strftime("%Y%m%d%I%M%S").to_s + '.csv'
      
      elsif params[:report_type] == "Activities"
        if params[:date_type].present?
          if params[:date_type] == "Specific Date"
            date = Date.strptime(params[:report_date],"%m-%d-%Y")
            @reports = @current_organization.activities.where(created_at: date.midnight..date.end_of_day).order("created_at desc")
          else
            first_date = Date.strptime(params[:report_date_first],"%m-%d-%Y")
            sec_date = Date.strptime(params[:report_date_sec],"%m-%d-%Y")
            @reports = @current_organization.activities.where(created_at: first_date.midnight..sec_date.end_of_day).order("created_at desc")
          end
        else
          @reports = @current_organization.activities.order("created_at desc")
        end
        filename = 'activities-' + Time.zone.now.strftime("%Y%m%d%I%M%S").to_s + '.csv'

      elsif params[:report_type] == "Users"
        if params[:admin_type].present?
          if params[:admin_type] == "Admin"
            @reports = @current_organization.users.where("admin_type =? OR admin_type =?",2,1).order("created_at desc")
          else
            @reports = @current_organization.users.where("admin_type =?",3).order("created_at desc")
          end
        else
          @reports = @current_organization.users.order("created_at desc")
        end
        filename = 'users-' + Time.zone.now.strftime("%Y%m%d%I%M%S").to_s + '.csv'
      end
      respond_to do |format|
        format.html
        format.csv { send_data params[:report_type] == "Contacts" ? Contact.to_csv(@current_organization.id, params[:cont_country]) : @reports.to_csv, :filename => filename }
      end
    rescue
