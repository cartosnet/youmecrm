...
#*****************************************************************************************/
  ...
  def index
    ...
    if user.present?
        ...
        count_total = Deal.includes(:deal_status).select("deals.id, deal_statuses.original_id").where(new_deal_count_condition).where("is_remote=0 and deal_statuses.original_id IN (?) ", [1]).count
        ...
        @total_new_deals = Deal.includes(:deal_status).select("deals.id, deal_statuses.original_id").where(user_condition).where("is_remote=0 and deal_statuses.original_id IN (?) ", [1]).count
#      @new_deals = get_deal_status_count([1], user, associated_by).count
  ...
  def get_incoming_deals
    ...
    params[:dtype] = params[:dtype].present? ? params[:dtype] : "incoming"
  ...
  def get_qualify_deals
    ...
    params[:dtype] = params[:dtype].present? ? params[:dtype] : "qualify"
    render :partial=> "deals/deal_other"
  end
  def get_not_qualify_deals
    #render :partial=> "deals/deal_not_qualify"
    params[:dtype] = params[:dtype].present? ? params[:dtype] : "not_qualify"
    render :partial=> "deals/deal_other"
  end
  def get_junk_deals
    #render :partial=> "deals/deal_junk"
    params[:dtype] = params[:dtype].present? ? params[:dtype] : "junk"
    render :partial=> "deals/deal_other"
  end
  ...
  def create
     contact = nil
      if !params[:deal][:contact_id].present?
        ...
        individual_contact[:country_id] = params[:deal][:country_id] if params[:deal][:country_id].present?
        begin
...
#        contact = Contact.find params[:deal][:contact_id]
      ...
      if contact.present?
        ...
        deal.save
        deal.update_column :country_id, params[:deal][:country_id]
...
def insert_opportunities deal   
  ...
  def update_deal_data(deal)  
    ...
    deal.tag_list = params[:deal][:tag_list] if params[:deal][:tag_list]
    deal.comments = params[:deal][:comments] if params[:deal][:comments]
    deal.probability = params[:deal][:probability] if params[:deal][:probability]
    deal.attempts  = params[:deal][:attempts] if params[:deal][:attempts]
...
# #    contact.save
  ...
  def move_deal
      ...
      if params[:mass_deal_ids].present?
         ...
         if (dm.deal_status.original_id == 4)
              ...
              c_name = current_user.full_name.present? ? current_user.full_name : ""
  ...
  def destroy  
   ...
   respond_to do |format|
      format.html { redirect_to return_url , notice: "Lead has been deleted successfully!"}
      format.js { render text: "success" }
    end
  end
 ...
 def fetch_activity
  ...
  def get_user_list_lead
   if params[:from].present? 
      ...
      data = @current_organization.users.where("invitation_token IS ? and is_active = ?", nil, true).select("id value, (case when id = " + @current_user.id.to_s + " then 'me' else ( case when first_name IS NOT NULL and first_name != '' and last_name IS NOT NULL and last_name != '' then CONCAT(first_name,' ',last_name) else email end) end) text").order("first_name")
   else
...
def edit_note
  ...
  def change_assigned_to
      if params[:value].present? && !params[:value].nil?
         ...
         # end
          assign_user_name = deal.assigned_user.present? ? (deal.assigned_user.full_name.present? ? deal.assigned_user.full_name : deal.assigned_user.email) : ""
          render text: assign_user_name
       end
  ...
  def change_lead_status
    ...
    lead = @current_organization.deals.find(params[:lead_id])
    lead.deal_status_id = status.id
    lead.save
    @deal_statuses = @current_organization.deal_statuses.where(is_kanban: true)
    render partial: "show_kanban_view"
  end
  ...
  def change_priority
    ...
    @lead.update_attributes :priority_type_id => params[:priority_id]
    @lead.save
    render text: @lead.priority_type.name
  end
  # Change the user label in lead listing page(Uncetegorised, Inbound or Outbound)
  def change_user_label
    @lead = @current_organization.deals.find(params[:id])
    user_lbl = UserLabel.where( name: params[:user_label_name] ).last
    @lead.update_attributes :user_label_id => user_lbl.id
    @lead.save
