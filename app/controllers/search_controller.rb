...
require 'will_paginate/array' 
  ...
  def search_all
    ...
    @deals = Deal.where("title like ? or contact_info like ?","%#{query}%", "%#{query}%" )
    
    @individual_contacts = IndividualContact.where("first_name like ? or last_name like ? or email like ?", "%#{query}%", "%#{query}%", "%#{query}%")
