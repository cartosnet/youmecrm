...
#  end
  ### Export CSV ###
  def self.to_csv(org_id,cont_country=nil)
    org = Organization.find_by_id org_id
    @all_contacts = org.individual_contacts + org.company_contacts

    if cont_country.present?
      filter_ind_contacts = org.individual_contacts.where("country_id =?", cont_country)
      filter_com_contacts = org.company_contacts.where("country_id =?", cont_country)
      @all_contacts = filter_ind_contacts + filter_com_contacts
    end

    p @all_contacts
    @contacts = @all_contacts.sort_by{|e| e[:created_at]}
    CSV.generate do |csv|
      csv << ["Name", "Email", "Country", "Type", "Last Touch", "Leads", "Tasks", "Last Lead", "Last Task"] ## Header values of CSV
      @contacts.each do |contact|
        ...
        csv << [cont_type == "company" ? contact.name : contact.full_name,
                contact.email, 
                contact.country.present? ? contact.country.name : "NA",
                cont_type,
                contact.updated_at,
