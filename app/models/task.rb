...
class Task < ActiveRecord::Base
  ...
  def self.to_csv
    CSV.generate do |csv|
      csv << ["Task Title", "Stage", "Created On", "Created By", "priority", "Next Action", "Contact Email", "Contact ID", "Contact Name"] ## Header values of CSV
      all.each do |task|
        deal = task.deal
        csv << [task.title, 
                ...
                deal.latest_task_type_id.present? ? deal.last_task.name  : "No-Action",
                deal.contacts_email,
                deal.contacts_id,
                deal.contacts_name ] ##Row values of CSV
      end
    end
  end
...
#    formated_event_end_date= self.event_end_date.in_time_zone(user.time_zone)
  ...
  def get_title_hover
    (self.taskable ? (self.taskable.title.size > 15 ? self.taskable.title[0..15] + "..." : self.taskable.title)  : "" ) + " / " + (self.title.size > 15 ? self.title[0..15] + "..." : self.title)
  end
